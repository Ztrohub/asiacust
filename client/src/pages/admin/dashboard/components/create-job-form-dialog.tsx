...
import { JOB_STATUS } from "@/shared/types/job";
...
interface CreateJobFormDialogProps {
	onJobCreated: () => Promise<void>;
}

export function CreateJobFormDialog({ onJobCreated }: CreateJobFormDialogProps) {
	...
	const onSubmit = async (values: z.infer<typeof registerSchema>) => {
		...
		const isConfirmed = await confirm({
			...
			description: 'Pekerjaan ini akan ditambahkan.'
		})
		...
		try {
			...
			const result = await api.post('/jobs', {
				...
				status: values.scheduleDate && values.workers && values.workers.length > 0 ? JOB_STATUS.SCHEDULED : JOB_STATUS.UNSCHEDULED
			})
